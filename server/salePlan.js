...
router.post('/queryForecastTableData', function (req, res) {
  sleep()
  let monthToSplit = req.body.monthToSplit,
    versionType = req.body.versionType,
    qmwd = req.body.timeDimension,
    level = req.body.level,
    year = req.body.year,
    childrenIdList = req.body.childrenIdList,
    childrenLevel = req.body.childrenLevel,
    parentId = req.body.parentId,
    parentLevel = req.body.parentLevel,
    count = 3,
    rs = {}, endMonth = 12, type = 'M', _sum = 0, _sumyoy = 0, _summom = 0
  if (qmwd === 'M') {
    rs.total = []
...
router.post('/queryAdjustTableData', function (req, res) {
  ...
  var count = childrenIdList.length
  ...
  if (qmwd === 'M') {
    ...
    let type = 'M'
    rs.total = []
    let _sum = 0, _otherSum = 0, _sumyoy = 0, _summom = 0
    for (var i = 1; i <= endMonth; i++) {
...
router.post('/queryAnalysisSaleStructureData', function (req, res) {
  ...
  // var childrenIdList = req.param('childrenIdList').split(",");
  var childrenLevel = req.body.childrenLevel
...
router.post('/queryForecastAnalysisCombinationData', function (req, res) {
  ...
  var year = 2018
  var qmwd = req.param('qmwd')
  var start = req.body.startDate.split('-')[1] * 1
  var end = req.body.endDate.split('-')[1] * 1
